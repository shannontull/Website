...
async def welcome(request: Request, db: Session=Depends(get_db)):
    ...
    figtem=px.bar(dfteam, x='Team',y='Salary',color='Fieldposition')update_xaxes(categoryorder="total descending")
    ...
    figpos.update_layout(yaxis=dict(tickfont=dict(size=5)),xaxis=dict(tickfont=dict(size5)),font=dict(size=5),margin=dict(l=0,r=0,t=0,b=0))
    ...
    pos10=dfteam.groupby('Fieldposition')['Salary'].mean().sort_values(ascending=False).head(10)
    pos10=pos10.reset_index()
    figpos2=px.box(dfteam.loc[dfteam['Fieldpostion'].isin(pos10.Fieldpostion)],x='Fieldposition',y='Salary',color_discrete_squence=['red'])
    ...
    dfteam=df.groupby('Fieldpostion')['Salary'].mean()
    dfteam=dfteam.reset_index()
    ...
    pospie=figpie.to_html(full_html=False,include_plotjs='cdn')
